...
// -----------------------------
// Content memory (localStorage)
// -----------------------------
const MEMORY_KEY = "bjj_content_memory_v1";
 ...
 *     "cta:global": ["...", "..."],
 *     "benefit:KIDS": ["...", "..."],
 *     "carousel:BEGINNER": ["Carousel Title", ...]
 *   }
 * }
 */
function loadMemory() {
 ...
 * If exhausted, it auto-resets that specific bucket and starts reusing.
 */
function chooseWithMemory(bucketKey, items, rng) {
  const mem = loadMemory();
  const usedArr = Array.isArray(mem.used[bucketKey]) ? mem.used[bucketKey] : [];
  const used = new Set(usedArr);

  let pool = items.filter((x) => !used.has(x));
  if (pool.length === 0) {
    // bucket exhausted: reset bucket only
    mem.used[bucketKey] = [];
    pool = items.slice();
  }
  ...
  // record chosen
  const nextArr = Array.isArray(mem.used[bucketKey]) ? mem.used[bucketKey] : [];
  nextArr.push(chosen);
  mem.used[bucketKey] = nextArr;
  saveMemory(mem);
...
function pickHook(pillar, rng) {
  // memory-aware: prefer unused hooks for that pillar
  return chooseWithMemory(`hook:${pillar}`, HOOKS[pillar], rng);
}

function formatCaption(pillar, ctx, rng) {
  const benefit = chooseWithMemory(`benefit:${pillar}`, BENEFIT_LINES[pillar], rng);
  const local = ctx.location.trim() ? `In ${ctx.location.trim()}.` : "";
  const cta = chooseWithMemory("cta:global", CTAS, rng);
...
function pickCarousel(pillar, rng) {
  ...
  const pool = candidates.length ? candidates : CAROUSEL_LIBRARY.filter((c) => c.pillar === "CULTURE");

  // memory-aware by title
  const titles = pool.map((c) => c.title);
  const chosenTitle = chooseWithMemory(`carousel:${pillar}`, titles, rng);
  return pool.find((c) => c.title === chosenTitle) || rng.choice(pool);
}
...
function generateMonth(ctx, baseVariation) {
  const seeds = (baseVariation == null)
    ? [1, 2, 3, 4]
    : [baseVariation, baseVariation + 1, baseVariation + 2, baseVariation + 3];

  return seeds.map((seed, i) => ({
    week_number: i + 1,
    variation: seed,
    result: generateWeek(ctx, seed),
  }));
}
...
function renderWeekHTML(result, prefixId = "") {
  ...
  const reels = ["CULTURE", "BEGINNER", "KIDS"]
    .map((key) => {
      const r = result.reels[key];
      return `
...
if (resetBtn) {
  resetBtn.addEventListener("click", () => {
    resetMemory();
    alert("Content memory reset. Next generation will reuse older ideas again.");
  });
}

// Optional defaults on load
